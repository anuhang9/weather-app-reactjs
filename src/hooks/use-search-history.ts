...
export function useSearchHistory(){
    const [history, setHistory] = useLocalStorage<SearchHistoryItems[]>("search-history", [])
    ...
    const addToHistory = useMutation({
        mutationFn: async(search: Omit<SearchHistoryItems, "id" | "searchedAt">)=>{
            const newSearch: SearchHistoryItems = {
                ...search, id: `${search.lat}-${search.lon}-${Date.now()}`, searchedAt: Date.now()
            }

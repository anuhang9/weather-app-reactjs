...
function FavoriteCityTablet({id, name, lat, lon, onRemove}: FavoriteCityTabletProps){
    ...
    return(
        <div onClick={()=>navigate(`/city/${name}?lat${lat}&lon${lon}`)} role="button" tabIndex={0} className="relative flex min-w-[250px] cursor-pointer items-center gap-3 rounded-lg border bg-card p-4 pr-8 shadow-sm transition-all hover:shadow-md">
            <Button variant={'ghost'} size={'icon'} onClick={(e)=>{e.stopPropagation(); onRemove(id); toast.error(`Removed ${name} from favorite.`)}} className="absolute right-1 top-1 h-6 w-6 rounded-full p-0 hover:text-foreground group-hover:opacity-100"><X className="w-4 h-4"/></Button>
            {isLoading?(<div className="flex h-8 items-center justify-center"><Loader className="h-4 w-4 animate-spin"/></div>): weather ? (<>

...
export const WeatherForecast = ({ data }: WeatherForecastProps) => {
  const daillyForecast = data.list.reduce((acc, forecast) => {
    ...
    return acc;
  }, {} as Record<string, DaillyForecastType>);

  const nextDay = Object.values(daillyForecast).slice(0, 6);
